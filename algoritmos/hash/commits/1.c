...
size_t fh(const  char *str, unsigned long  tam)  {
        size_t hash = 0;
        int c;
...
hash_nodo_t* paquete_crear(const char* clave, void*  dato){
...
bool almacenar(hash_t* hash, lista_t** tabla, hash_nodo_t* paquete, size_t largo){
  bool resu = true;
  size_t indice = fh(paquete->clave, largo );
  if (!tabla[indice]){
...
bool hash_guardar(hash_t *hash, const char *clave, void *dato){
  bool res;
  hash_nodo_t* paquete = paquete_crear(clave, dato);
  if(!paquete) return NULL;
  if (hash->tam > 3 * hash->largo) {
    size_t  nuevo_largo = ( hash->largo * 3);
...
void *hash_borrar(hash_t *hash, const char *clave){
  size_t indice = fh(clave,hash->largo);
...
void lista_destruir2(lista_t* lista, hash_destruir_dato_t destruir_dato){ // Funcion que cree parecida a la de la lista, ya que la otra no me servia
  ...
  hash_nodo_t* aux2 = lista_borrar_primero(lista);
  free(aux2->clave);
  free(aux2);
  lista_destruir2(lista, destruir_dato);
}
