...
// crea una cola 
// post : devuelve una cola alocada en memoria dinamica
...
bool cola_esta_vacia(const cola_t *cola){
  if (!cola->ultimo && !cola->primero) return true;
  return false;
}

// Devuelve una  nodo tipo nodo_t* alocado en memoria dinamica
// Post: Se devuelve un nodo  
...
bool cola_encolar(cola_t *cola, void* valor){
  ...
  if (!cola_esta_vacia(cola))	cola->ultimo->siguiente= nodo;
...
void* cola_desencolar(cola_t *cola){
  if (!cola_esta_vacia(cola)){
    void* dato_salida=cola->primero->dato;
    void*aux=cola->primero->siguiente;    		       
    free(cola->primero);
    cola->primero=aux;
